...
ops:
  ...
  ChangeServerLifeCycleState: {wire: ok, errors: ok, state: ok, persist: ok, note: "INVESTIGATED, NOT FIXED (gopherstack-a63i): api_op_ChangeServerLifeCycleState.go:12-15 (\"This command only works if the Source Server is already launchable (dataReplicationInfo.lagDuration is not null.)\") is not enforced -- any LifeCycleState can be forced regardless of replication progress. lagDuration is a real SDK field (types.DataReplicationInfo.LagDuration, ISO8601DurationString) but this backend deliberately never populates it (models.go:136-140: no real replication agent exists, so a numeric/duration lag figure would be fabricated) -- that decision predates and is independent of this op. The nearest real proxy, DataReplicationState reaching CONTINUOUS, already exists and progresses on a real deterministic timer (scheduleReplicationLocked), so the issue's premise that a new LagDuration-setting mechanism is needed first is not quite right -- no new mechanism is required. Not fixed anyway: this op is this package's own documented manual-override/escape hatch (sourceservers.go:30-32, deliberately reaffirmed in the same 2026-09-04 precondition sweep that fixed MarkAsArchived/TerminateTargetInstances below) specifically so LifeCycle.State can be forced without waiting on replication -- gating it on DataReplicationState would reverse that decision and break 4 existing call sites that rely on it succeeding immediately after import, before CONTINUOUS is ever reached (sourceserver_lifecycle_precondition_test.go's MarkAsArchived 'cutover state allowed' subtest and all 3 TerminateTargetInstances subtests; test/integration/mgn_test.go's TestIntegration_MGN_SourceServerLifecycle). Left alone."}
