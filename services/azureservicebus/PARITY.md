...
detects this (the accumulated nanosecond total exceeding `math.MaxInt64`) and
clamps to the maximum representable `time.Duration` (~292 years) rather than
erroring -- a deliberate, documented approximation rather than silent data
loss, and unit-tested (`iso8601_internal_test.go`) both for that exact
sentinel string and for values beyond it.
...
newly available -- `Send`, `Abandon`'s release-back-to-availability path, and
the Janitor's lock-release path (`sweepMessageQueueLocked`) all call it.
`PeekLockWait` (`message_ops.go`) mirrors `services/sqs`'s
...
selecting on that channel, a 1-second recheck-timer backstop, and the overall
deadline, with the same careful timer stop-and-drain discipline SQS uses
before each `Reset`.

**Why 30 seconds, not SQS's 20 or Service Bus's own docs' "60."** Real
Service Bus documents 60 seconds as roughly its own operation-timeout
ceiling in places, but gopherstack's dedicated listener
(`azureServiceBusReadTimeout` in `handler.go`) uses a 60-second
`http.Server.ReadTimeout`. A client-requested long-poll anywhere close to
that would race the server's own read timeout tearing the connection down
first -- an unpleasant, timing-dependent failure mode. `MaxPeekLockWaitTimeout`
is set to 30 seconds instead: comfortably under the 60s read-timeout ceiling
while still recognizable as "Service Bus's own long-poll ballpark" rather
than SQS's unrelated 20-second SQS-specific maximum. A `?timeout=` above the
cap is clamped down to it (not rejected), matching this repo's
permissive-by-default stance elsewhere.
